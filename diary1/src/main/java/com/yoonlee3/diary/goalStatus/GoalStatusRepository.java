...
public interface GoalStatusRepository extends JpaRepository<GoalStatus, Long>{
	...
	// 끝난 목표의 성공 수 구하기
	@Query("select count(gs) from GoalStatus gs where gs.goal.id= :goal_id and "
			+ "gs.createDate = :start and gs.createDate = :end and gs.is_success = true")
	int findStatusDay(Long goal_id, LocalDate start, LocalDate end);
